...
// ==========================================
    ...
    async function handleUpload(photo, filename, sourceDesc, categoryId = 0, customTitle = '', thumbnailData = null, dim = '') {
      ...
      const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/avif', 'image/bmp', 'image/x-icon', 'image/vnd.microsoft.icon', 'image/svg+xml'];
      if (!allowedTypes.includes(photo.type)) {
        throw new Error("非法文件类型：仅允许上传 JPG/PNG/GIF/WEBP/AVIF/BMP/ICO/SVG 格式");
      }
      
      if (photo.size > 5 * 1024 * 1024) {
        throw new Error("文件体积过大：不可超过 5MB");
      }
